...
func (s *ProxyServer) createProxier(ctx context.Context, config *proxyconfigapi.KubeProxyConfiguration, dualStackMode, initOnly bool) (proxy.Provider, error) {
	...
	if dualStackMode {
		...
		var localDetectors [2]pf.LocalTrafficDetector
		...
		var localDetector pf.LocalTrafficDetector
...
func getLocalDetector(ipFamily v1.IPFamily, mode proxyconfigapi.LocalMode, config *proxyconfigapi.KubeProxyConfiguration, nodePodCIDRs []string) (pf.LocalTrafficDetector, error) {
	switch mode {
	case proxyconfigapi.LocalModeClusterCIDR:
		...
		if len(cidrsByFamily[ipFamily]) != 0 {
			return pf.NewDetectLocalByCIDR(cidrsByFamily[ipFamily][0].String())
		}
	...
	case proxyconfigapi.LocalModeBridgeInterface:
		return pf.NewDetectLocalByBridgeInterface(config.DetectLocal.BridgeInterface)

	case proxyconfigapi.LocalModeInterfaceNamePrefix:
		klog.InfoS("Detect-local-mode InterfaceNamePrefix not supported on FreeBSD")
	}

	klog.InfoS("Defaulting to no-op detect-local")
	return pf.NewNoOpLocalDetector(), nil
}

func getDualStackLocalDetectorTuple(mode proxyconfigapi.LocalMode, config *proxyconfigapi.KubeProxyConfiguration, nodePodCIDRs []string) ([2]pf.LocalTrafficDetector, error) {
	var localDetectors [2]pf.LocalTrafficDetector
	var err error
