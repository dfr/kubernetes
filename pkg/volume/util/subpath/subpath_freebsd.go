...
const (
	// This symbol is not in unix package because O_PATH is not supported in
	// 12.x
	O_PATH = 0x00400000
	...
	openFDFlags = unix.O_NOFOLLOW | O_PATH
)
...
func doBindSubPath(mounter mount.Interface, subpath Subpath) (hostPath string, err error) {
	// On FreeBSD, we can't mount file descriptors to the target. This
	// implementation just mounts subpath to the target - this should be
	// safe since the container can't change or remove the target once mounted.
	...
	options := []string{}
	mountFlags := []string{}
	klog.V(5).Infof("nullfs mounting %q at %q", subpath.Path, bindPathTarget)
	if err = mounter.MountSensitiveWithoutSystemdWithMountFlags(subpath.Path, bindPathTarget, "nullfs", options, nil /* sensitiveOptions */, mountFlags); err != nil {
...
func doSafeMakeDir(pathname string, base string, perm os.FileMode) error {
	...
	for _, dir := range toCreate {
		...
		err = unix.Mkdirat(parentFD, currentPath, uint32(perm))
		if err != nil {
			return fmt.Errorf("cannot create directory %s: %s", currentPath, err)
		}
		// Dive into the created directory
		childFD, err = unix.Openat(parentFD, dir, nofollowFlags|unix.O_CLOEXEC, 0)
		if err != nil {
...
func findExistingPrefix(base, pathname string) (string, []string, error) {
	...
	for i, dir := range dirs {
		...
		// fifo
		childFD, err := unix.Openat(fd, dir, O_PATH|unix.O_CLOEXEC, 0)
		if err != nil {
			if os.IsNotExist(err) {
...
func doSafeOpen(pathname string, base string) (int, error) {
	...
	for _, seg := range segments {
		...
		childFD, err = unix.Openat(parentFD, seg, openFDFlags|unix.O_CLOEXEC, 0)
		if err != nil {
